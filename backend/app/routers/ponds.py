...
from app.services.microscopy import open_batch_pond_ids

router = APIRouter(prefix="/api/ponds", tags=["ponds"])


def _attach_open_flag(db: Session, ponds: List[Pond]) -> List[Pond]:
    open_ids = open_batch_pond_ids(db, [p.id for p in ponds])
    for p in ponds:
        p.has_open_microscopy = p.id in open_ids
    return ponds
...
def list_ponds(
    ...
    return _attach_open_flag(db, ponds)
...
def get_pond(
    ...
    return _attach_open_flag(db, [item])[0]
